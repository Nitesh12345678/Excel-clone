...
$(document).ready(function () {
    for (let i = 1; i <= 100; i++) {
        ...
        let colname = $(`<div class="column-name colId-${i}" id="colCod-${ans}" >${ans}</div>`);
        $(".column-name-container").append(colname);
        let rowname = $(`<div class="row-name" id="rowId-${i}">${i}</div>`);
        $(".row-name-container").append(rowname);
    }

    for (let i = 1; i <= 100; i++) {  //create row
        let row = $(`<div class="cell-row"></div`);
        for (let j = 1; j <= 100; j++) {
            //create cell one by one
            let columnCode = $(`.colId-${j}`).attr("id").split("-")[1];
            let column = $(`<div class="input-cell" contenteditable="false" id="row-${i}-col-${j}" data=" colCode-${columnCode}"></div>`);
            //append cell to row to make it row 
            row.append(column);
        }
        // append row to container to make it matrix
        $(`.input-cell-container`).append(row);
    }

    // when user click on div which as align-icon class the following action should take place
    $(".align-icon").click(function () {
        // diselect the previously selected class
    ...
    // on double click on any cell perform following actions
    $(".input-cell").dblclick(function () {
        // remove selected class form other
        $(".input-cell.selected").removeClass("selected");
        // add selected class on current
        $(this).addClass("selected");
        // make content editable true
        $(this).attr("contenteditable", "true");
        // bring focus on current cell to type 
        $(this).focus();
    });
    ...
    // on scroll of input-cell-container scroll column-name-container too
    $(".input-cell-container").scroll(function () {
        //scroll left for scroll to   left to right side 
        $(".column-name-container").scrollLeft(this.scrollLeft);
    });

    // on scroll of input-cell-container scroll row-name-container too
    $(".input-cell-container").scroll(function () {
        // scrollTop for scrolling top to bottom and vice versa
    ...
    function changeHeader(ele) {
        ...
        $(".font-familly-selector").val(cellInfo["font-family"]);
        $(".font-familly-selector").css("font-family", cellInfo["font-family"]);
    ...
    // on click on color-fill-icon also click on background -color-picker
